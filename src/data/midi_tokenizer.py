...
class MT3Tokenizer:
    ...
    def _vel_to_bin(self, velocity: int) -> int:
        ...
        if velocity == 0:
            return 0
        return math.ceil(self.NUM_VELOCITY_BINS * velocity / config.midi_max)

    def _bin_to_vel(self, bin_val: int) -> int:
        """Inverse of _vel_to_bin: vocabulary bin index → MIDI velocity."""
        if bin_val == 0:
            return 0
        return round(bin_val * config.midi_max / self.NUM_VELOCITY_BINS)
    ...
    def decode(self, tokens: List[int]) -> pretty_midi.PrettyMIDI:
        ...
        for tok in tokens:
            ...
            elif self._VEL_BASE <= tok <= self._VEL_BASE + config.midi_max:
            ...
            elif self._PITCH_BASE <= tok <= self._PITCH_BASE + config.midi_max:
            ...
            elif self._PROG_BASE <= tok <= self._PROG_BASE + config.midi_max:
                # Select instrument for subsequent pitch tokens.
                cur_program = tok - self._PROG_BASE
                is_drum = False
                cur_steps = 0  # reset accumulator after non-shift token

            elif self._DRUM_BASE <= tok <= self._DRUM_BASE + config.midi_max:
                # Drum hits have no matching note-off; we assign a fixed duration.
                pitch = tok - self._DRUM_BASE
                completed.append((cur_time, cur_time + config.drum_duration_s, pitch, 0, cur_velocity, True))
                cur_steps = 0  # reset accumulator after non-shift token
        ...
        for (start, end, pitch, prog, vel, drum) in completed:
            ...
            note = pretty_midi.Note(
                ...
                pitch=pitch,
                start=start,
                end=max(end, start + config.min_note_duration_s),
            )
